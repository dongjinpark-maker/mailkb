...
# Windows 파이썬만 쓰는 이유는 **본문에** 둔다(2026-09-13). 주석에만 두었더니 PowerShell 이 실패했을 때
# 모델이 붙잡을 전제가 없어 python3 로 우회했다. 실측: WSL python3 → Windows 디스크 DB 는 Windows
# 프로세스가 쥐고 있으면 disk I/O error(12/12), Windows 파이썬 → WSL 디스크 DB 는 혼자여도 locked(4/4).
...
permission:
  ...
  bash:
    ...
    "*sqlite*": deny
    # WSL 파이썬으로 mailkb 를 직접 돌리지 못하게 — PowerShell 형식은 powershell.exe 로
    # 시작하므로 걸리지 않는다. python.exe(Windows 파이썬)도 걸리지 않는다.
    "python -m mailkb*": deny
    "python3 -m mailkb*": deny
  # 시스템 임시 자리에 쓰지 않는다 — 스크래치는 저장소 루트의 temp/ 하나로 몬다.
...
mailkb 는 반드시 **Windows 에서 도는 파이썬**(아래 PowerShell 형식)으로만 실행한다.
이유: DB 는 Windows 디스크에 있고 Windows 쪽 웹 서버가 늘 열어 둔다. WSL 의 python·python3 가
같이 열면 WAL 잠금이 WSL↔Windows 경계를 못 넘어 `disk I/O error` 가 난다.
DB 파일(`*.sqlite`)은 직접 열지 않는다.
