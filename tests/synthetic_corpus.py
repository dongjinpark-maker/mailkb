...
class _CorpusBuilder:
    ...
    def _quote(parent: MailRecord, korean: bool) -> str:
        """실제 Outlook 답장 인용 — 형태를 섞는다.

        실기기의 body_text 는 HTMLBody→마크다운 변환 결과라 헤더 라벨이
        **굵게**(`**보낸 사람:**`) 오고, 구분선도 밑줄이 아니라 테두리 div 라
        마크다운에 아무것도 안 남는 경우가 흔하다. 코퍼스가 밑줄+평문 라벨만
        만들면 절단 회귀를 영영 못 잡는다(2026-07-31 실제로 놓쳤다)."""
        # 형태는 부모 메일별로 갈린다(MailRecord 엔 sequence 가 없다 —
        # hasattr 로 물어보던 코드는 항상 거짓이었다).
        shape = (len(parent.message_id) + len(parent.subject)) % 3
        if korean:
            if shape == 0:                       # 클래식 — 밑줄 구분선 + 평문
                return (
            ...
            return (                             # 해외 파트너(프랑스어 클라이언트)
                f"\n\n**De :** {parent.sender_name} <{parent.sender_addr}>\n"
                f"**Envoyé :** {parent.sent_on}\n"
                f"**À :** {'; '.join(parent.to)}\n"
                f"**Objet :** {parent.subject}\n\n{parent.body_text}"
            )
        if shape == 1:                           # 굵은 라벨(영문)
            return (
                f"\n\n**From:** {parent.sender_name} <{parent.sender_addr}>\n"
    ...
    def _html_body(authored: str, parent: MailRecord | None) -> str:
        ...
        if parent is not None:
            # 실제 클라이언트 구조를 번갈아 낸다 — 라벨형만 만들면 새 HTML
            # 경계 판정(border-top·hr·전용 컨테이너)이 회귀 테스트에서 한
            # 번도 안 돌아간다(2026-07-31 리뷰).
            hdr = (f"<b>보낸 사람:</b> {html.escape(parent.sender_name)}<br>"
                   f"<b>보낸 날짜:</b> {parent.sent_on}<br>"
                   f"<b>받는 사람:</b> {html.escape('; '.join(parent.to))}<br>"
                   f"<b>제목:</b> {html.escape(parent.subject)}")
            body = f"<blockquote>{html.escape(parent.body_text)}</blockquote>"
            shape = (len(parent.message_id) + len(parent.subject)) % 3
            if shape == 0:
                parts.append("<div style='border:none;border-top:solid #E1E1E1 "
                             f"1.0pt;padding:3.0pt 0cm 0cm'><p>{hdr}</p>{body}</div>")
            elif shape == 1:
                parts.append(f"<hr><div id='divRplyFwdMsg'>{hdr}</div>{body}")
            else:
                parts.extend([
