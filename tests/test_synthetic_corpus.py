...
class TestSyntheticCorpus(unittest.TestCase):
    ...
    def test_is_substantially_larger_than_the_interactive_demo(self):
        # 이 코퍼스는 분류 정확도를 재는 벤치마크라 라벨별 개수가 고정돼 있다
        # (위 test_thread_labels_are_stable). 데모(fake)는 기능 시연용이라 자유롭게
        # 커지므로, 여기서는 '벤치마크가 데모보다 충분히 크다'만 지킨다 —
        # 둘이 같은 데이터를 가리키거나 코퍼스가 쪼그라드는 것을 잡는 가드.
        demo_count = len(list(FakeSource().fetch(None)))
        ratio = len(self.corpus.records) / demo_count
        self.assertGreaterEqual(ratio, 3.0)
        self.assertGreater(len(self.corpus.records), 900)
