...
> **P2(2026-07-15)**: AI 흐릿한 기억 검색 — 번역→검색→본문심사(+자기교정).
> **P2-lat(2026-07-15)**: 지연 개선 — 재순위+확정을 본문심사 1콜로 병합(3→2콜) ·
> 백그라운드 잡+폴링(서버 안 멈춤·단계 스트리밍) · 잠정 결과 먼저 보여주고 확정.
> 제약(사용자 확정): **로컬 · stdlib 전용 · 임베딩/RAG 범위 밖**. skill 추출은 뒤 단계.
> 노브 확정: 제목:본문 = **3:1** · 완화 4단계 · 관련도 기본(OR만 '관련 낮음') ·
> AI=sonnet · 캐시 지속 · DSL 표시·편집 · **본문심사 상위 12건**(구 심층읽기 5건 대체).
...
| **Phase 2** | AI 계층 = 자연어→DSL 번역 + 유의어 확장 + 반복 + 후보 우선순위 정리(본문 합성 없음) | **완료** |
...
## 7. Phase 2 — 흐릿한 기억 (RAG 없음)

**구현됨 (`review.ai_search`)**. 진입은 명시적 opt-in — 일반검색(무료) 결과 위
`🤖 AI로 다시 찾기` 버튼(웹) / `search --ai`(CLI). 파이프라인(AI 호출 **보통 2회**):
...
4. **본문심사**(sonnet) 엔진 상위 **12건**의 본문까지 한 콜에 읽어 순위+확정+이유를
   한 번에 낸다(구 재순위+심층읽기 통합 — `_judge_bodies`→`ai_confirm_top`). 이유는
   이 최종 단계에서만 생성(중간 재순위 이유 생략 = 출력·지연 절감).
5. **자기교정** 확정 결과 0이면 재번역·재검색·재심사 **1회**(그때만 4콜).

**지연 개선(P2-lat)**: ① 재순위(스니펫 30건)+심층읽기(본문 5건) → 본문심사 1콜(상위
12건 정독)로 병합해 3콜→2콜. 정독 대상이 5→12로 늘어 정확도는 오히려 강화. ② **백그라운드
잡+폴링**(`_start_aisearch`+`/aisearch/status`) — 요청 스레드를 막지 않아 서버가 안 멈추고,
단계(번역→검색→본문심사)를 실시간 스트리밍. ③ **잠정 결과** — 본문 읽기 전 엔진 1차
후보를 먼저 흐리게 보여주고, 확정되면 자동 교체(`progress` 콜백의 `prelim`).

AI 는 **DSL만** 출력하고 파서가 정화(원시 SQL 금지). CLI 부재·타임아웃은 `AIError`→잡이
받아 `/aisearch/status`가 일반검색 폴백. 해석 DSL 은 화면에 노출·편집 가능(오해석을 AI
재호출 없이 교정). **범위**: '찾는 메일' 판단까지 — 답 합성(지식검색)은 별도.
...
| `mailkb/web.py` | 헤더 검색창·`render_search`·`render_aisearch`·`/search?ai=1` + 백그라운드 잡(`_start_aisearch`·`render_aisearch_status`·`/aisearch/status`)·폴링(`hookAiPolling`) |
| `mailkb/cli.py` `cmd_search` | 스니펫 출력 + `--json` + `--ai` |
| `mailkb/review.py` (P2) | `ai_translate_query`·`_judge_bodies`·`ai_confirm_top`(본문심사)·`ai_search`(`progress` 콜백) + 프롬프트 |
