...
> 상태: **Phase 1 구현 완료 (2026-07-14)** — DSL 파서(`search.py`) + 단계적
> 엔진(`store.search`) + 스마트 검색 UI(`render_search`) + CLI(`--json`).
> 제약(사용자 확정): **로컬 · stdlib 전용 · 임베딩/RAG 범위 밖**. Phase 2(흐릿한
> 기억 = 자연어→DSL→엔진→LLM 우선순위)와 skill 추출은 뒤 단계.
...
AI 가 자연어 요청을 받아: ① 구조 제약 추출(사람→`from:`, 시점→`after:`/`before:`,
첨부→`has:`) ② 키워드 추출 후 **한↔영·유의어 확장** ③ 엄격→완화로 `--json` 검색 반복
(≤3회) ④ 후보의 제목·발신·스니펫만으로 **상위 3건 + 한 줄 이유** 제시(본문 통독·합성
없음 — "여기 있는 것 같아요"). 엔진의 tier 완화 + AI 의 의미 재구성 = 2겹.
...
| `mailkb/web.py` `render_search` 등 | 스마트창·상세 병합·패싯·스니펫 렌더 |
| `mailkb/cli.py` `cmd_search` | 스니펫 출력 + `--json` |

## 10. 남은 것 (Phase 1 후속)

- 검색창을 셸 헤더로 승격(현재 `/search` 페이지 상단). 지금은 nav '검색' 링크 경유.
- bm25 recency 블렌드는 현재 '동점 시 최신순' 정도 — 필요 시 `−bm25 + w·exp(−age/τ)`
  시간감쇠로 강화(τ 는 skill 파라미터).
- 실데이터로 제목:본문 3:1 · 완화 임계 튜닝.
