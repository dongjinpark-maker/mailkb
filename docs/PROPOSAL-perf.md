...
- 파생 버전(`_feature_version`/`_action_version`): 규칙·내 주소·이름이 바뀌면 `message_features`
  재계산, 노이즈 설정(차단 등)만 바뀌면 액션 상태만 재접기 — 둘 다 기존 `messages`에서 1회
  로컬 백필(재수집·DB 삭제 불필요). `serve()`의 keep-alive Store가 브라우저를 열기 전에 수행.
