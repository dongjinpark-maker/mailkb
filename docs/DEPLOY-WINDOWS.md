...
| `[ai] diagnose` | 현안 브리핑 백엔드 (기본 `opus`) | 스레드·인물 화면의 [현안 브리핑]이 쓴다. **쓰는 CLI 가 opus 를 못 부르면 이 버튼만 실패한다** — `mailkb diagnose` 가 실제 호출로 확인해 주고, 웹 **설정 › 현안 브리핑 백엔드**에서 sonnet 으로 바꿀 수 있다 |
...
- [ ] AI를 쓸 계획이면 `mailkb diagnose`로 백엔드 **응답** 확인 — 역할이 쓰는
      ...
      여기서 드러난다** — 그때는 웹 설정 › 현안 브리핑 백엔드를 sonnet 으로.
      터미널을 안 열 거면 **웹 설정 › AI 백엔드 상태**의 [응답 시험]이 같은 일을
