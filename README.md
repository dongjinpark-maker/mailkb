...
- **오늘 메일 정리 (AI 6단계)**: 누적 요약 갱신 → **결정·신호 수확** → 핵심 한 줄 →
  개입 분류 → 우선순위 주석 → **하루 요약**(맨 위 3~5문장). 대기 화면에 진행 바(단계 i/6)와
  애니메이션. 하루 이틀 건너뛰어도 다음 실행이 소급한다(워터마크). AI 없이 "기록만 남기기"도 가능.
...
- **인물 도시에**: 최근 3개월 교류 강도순 인물 목록 → 사람별 카드(관계 수치·진행 중·
  서로의 미결·관여한 결정·최근 변화·**주요 어휘**). 이미 추출된 메일·액션·결정·신호를
  사람 중심으로 재조립한 결정론 화면(v1, AI·새 테이블 없음). '주요 어휘'는 본인 발신어
  빈도 태그 클라우드 — 도메인 어휘가 드러나 "관여 영역"을 보여준다. 스레드에서 이름 클릭으로도 진입.
