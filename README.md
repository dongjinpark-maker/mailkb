...
python3 tests/test_mailkb.py                     # 단위 테스트 302건 (외부 의존 없음)
```
...
클릭 한 번에 ① `git pull`(최신 소스, 오프라인·충돌이면 현재 코드로 진행) ② 떠 있던
서버 종료 ③ 서버 시작 ④ Edge 앱 창 열기까지 하고, **그 창을 닫으면 서버도 함께
내려간다**(콘솔 없음 — pythonw). 재시작·종료 버튼은 없다: 다시 아이콘을 누르면
pull 후 새 서버로 뜬다.
