...
- 회신·결정 신호는 증거 강도 × 수신 대상 사다리로 판정 — 직접 수신·이름 지목·
  내 참여는 '회신 필요', 그룹·대량 발송·외부 발신은 접힌 '확인 후보'로 강등해
  단체 공지(수신 50명 전사 메일 포함)의 "회신 바랍니다"·"금일 18시까지" 류가
  개인 액션 목록을 오염시키지 않음
...
python3 -m unittest discover -s tests -p 'test_*.py'  # 단위 테스트 508건 (외부 의존 없음)
...
- **메일함·스레드 공통 필터 탭**: 전체 · 미개봉 · ↩ 회신 필요 · ⏰ 기한 ·
  🚩 플래그 · 🙈 숨김. 열람 시 읽음(볼드 해제), 키보드 `j/k` 이동 ·
  `x` 신호 끄기·복원(회신 필요·확인 후보) · `f` 플래그 · `h` 숨김 · `/` 검색 —
  `x/f/h` 는 우측에 열린 스레드에, 없으면 목록 커서 행에 동작(상태명 토스트,
  필터 바 오른쪽 ⓘ 안내). 스크롤 추가 로딩, 자동 동기화(기본 30분 — 새 메일 오면 토스트).
...
- **인물 도시에**: 최근 6개월 교류 강도순 인물 목록 → 사람별 카드(관계 수치·진행 중·
  관여한 결정·최근 변화·**업무 어휘 지도**). 이미 추출된 메일·액션·결정·신호를
...
| `search <질의> [--ai --json]` | 전문검색 (FTS5 trigram; `--ai` 흐릿한 기억 검색, `--json` 구조화 출력) | 선택 |
| `show <번호>` / `thread <ID>` | 본문(인용 제거본) / 스레드 타임라인 | ✗ |
| `act [--ai --note ..]` | 지금 할 일 큐 (결정론 4분류; --ai 로 재분류·우선순위·주석) | 선택 |
| `audit [--label --report]` | 분류 판정 감사 — 실메일 라벨링·혼동 행렬 | ✗ |
