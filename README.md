...
- **오늘 메일 정리 (AI 7단계)**: 누적 요약 갱신 → **결정·신호 수확** → 핵심 한 줄 →
  개입 분류 → 우선순위 주석 → **하루 요약** → **인물 도시에 갱신**. 대기 화면에 진행 바(단계 i/7)와
...
- **인물 도시에**: 최근 3개월 교류 강도순 인물 목록 → 사람별 카드(관계 수치·진행 중·
  ...
  사람 중심으로 재조립한 결정론 화면. '주요 어휘'는 본인 발신어 빈도 태그 클라우드 —
  도메인 어휘가 드러나 "관여 영역"을 보여준다. `--ai` 시 맨 위에 **AI 요약 카드**(역할·
