...
mailkb는 Outlook이 못 하는 것만 한다: **아카이브 인덱스, 스레드 지식화, 하루 끝 회고, "개입 필요" 큐.**
...
python3 -m mailkb --home ./demo sync            # fake 소스: 한국어 합성 메일 189통 (최근 1개월, SoC·AI·보안)
                                                 # (긴 스레드 12~14통, 수신50명 전사메일, 스팸, 시스템 노티,
                                                 #  정체·그룹결정 사례 포함)
python3 -m mailkb --home ./demo ls --unanswered
python3 -m mailkb --home ./demo act              # 개입 필요 큐 (결정론)
...
python3 tests/test_mailkb.py                     # 단위 테스트 241건 (외부 의존 없음)
```

## 웹 UI — Minerva

`mailkb serve` 는 Outlook 유사 **좌/우 분할**의 로컬 웹 앱이다 — 왼쪽은 목록
(홈·메일함·스레드·검색·기록·설정), 오른쪽은 읽기 패널. 스레드를 열면
...
- **홈 = "지금 할 일"**: 결정론 개입 큐 4분류 — 🔴 내 결정 대기 / 🟠 내 응답 대기 /
  🟡 내가 넘긴 공(정체, 영업일) / ⚪ 멈춘 스레드. 나를 지목한 항목은 ★ 우선.
  아래에 오늘 메일 핵심(업무 스레드 한 줄씩)과 장기기억 카운트.
- **메일함·스레드 공통 필터 탭**: 전체 · 미개봉 · ↩ 내 응답 대기 · ⏰ 기한/요청 ·
  🚩 플래그 · 🙈 숨김. 열람 시 읽음(볼드 해제), `j/k` 키보드 이동, 스크롤 추가 로딩,
  자동 동기화(기본 30분 — 새 메일 오면 토스트).
- **오늘 메일 정리 (AI 5단계)**: 누적 요약 갱신 → **결정·신호 수확** → 핵심 한 줄 →
  개입 분류 → 우선순위 주석. 대기 화면에 진행 바(단계 i/5)와 애니메이션.
  하루 이틀 건너뛰어도 다음 실행이 소급한다(워터마크). AI 없이 "기록만 남기기"도 가능.
- **장기기억 (결정의 영구 기억)**: 수확이 메일 원문에서 결정을 추출해 **반영 대기**
  초안으로 올리고(원문 인용 검증 — 환각 차단), **반영/유보는 사람이 결정**한다
  (기록 › 장기기억). 스레드의 `장기기억` 버튼으로 수동 기록(즉시 반영).
  반영된 것만 검색·회고 재료가 된다.
- **기록**: 데일리(◀ ▶ 날짜 이동) · 장기기억(반영/유보 목록·검색·상호 복구).
- **통계**: 기간(2/4/8/16주) 신호 대시보드 — 증발한 내 요청 · 조용해진 사람 ·
  내 응답 지연 추세 · 야간/주말 발신 · 자주 주고받는 상대(관계 그래프).
- **설정**: 라이트/다크 테마(다크는 웜 코랄) · 판정 기준·노이즈 규칙 런타임 편집
  (`overrides.json` 에 영구 — config.toml 무손상).
...
### 첫 실행 시 확인할 것 (COM 어댑터는 실 Outlook 미검증)
...
| `act [--ai --note ..]` | 개입 필요 큐 (결정론 4분류; --ai 로 재분류·우선순위·주석) | 선택 |
| `review` | 데일리 리뷰: **오늘 메일 핵심**(스레드 한 줄, 공지/스팸 제외)·보낸 것·개입 필요·미답변·기한 → vault/daily | ✗ |
| `review --ai` | + 누적 요약 갱신 + **결정·신호 수확(장기기억 제안)** + 핵심 AI 한 줄 + 개입 큐 분류·주석 | ✓ |
...
| `serve [--port --open --app]` | Minerva 웹 UI (localhost, 위 참고) | ✗ |
