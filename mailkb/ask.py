...
ASK_FEATURE_VERSION = 4   # 쟁점 분석 issues[] — 골격을 answer 밖 전용 필드로
...
ASK_NOTES_TOP = 3         # 프롬프트에 실을 내 노트 상한 (직접 지목 스레드는 무조건)
ASK_NOTE_CHARS = 500      # 노트 한 건당 싣는 길이 — 템플릿상 요지가 앞에 온다
ASK_KNOWLEDGE_TOP = 3     # 프롬프트에 실을 지식 상한 — 노트와 같은 결정론 선정
ASK_KNOWLEDGE_CHARS = 600 # 지식 한 건당 싣는 길이(보강된 본문이라 노트보다 김)
...
def _verify(rows, read: dict, keys=("text",), pad: int = QUOTE_CONTEXT) -> list[dict]:
    ...
    for it in rows or []:
        ...
        try:                               # 쟁점 분석의 근거-쟁점 연결(1-based)
            item["issue"] = int(it["issue"])
...
_ISSUE_STATES = ("합의", "해소", "진행 중", "보류", "평행선")


def _clean_issues(rows, claims: list[dict]) -> list[dict]:
    """쟁점 분석 골격 — 검증 통과 근거가 하나도 없으면 쟁점 목록도 내지 않는다
    (headline 과 같은 원칙: 근거 없는 결론은 내지 않는다).

    이 필드가 answer 와 분리된 이유: 쟁점명·상태는 여러 메일을 종합한 판정이라
    인용 하나로 증명되지 않아, answer 에 실으면 검증기가 기각하고 재작성·폴백이
    구조를 지운다(2026-08-09 실측). status 는 어휘 밖이면 비운다 — 틀린 판정
    칩을 다는 것보다 칩이 없는 편이 낫다."""
    if not claims:
        return []
    out = []
    for it in rows or []:
        ...
        if not title:
            continue
        status = " ".join(str(it.get("status") or "").split())
        if status == "진행중":
            status = "진행 중"
        if status not in _ISSUE_STATES:
            status = ""
        out.append({"title": title[:80], "status": status,
...
def _notes_block(store: Store, question: str, deny: frozenset,
                 allow: set | None = None) -> str:
    """내 노트(vault/notes 색인) 중 관련분 — 문맥 전용 블록('' 이면 생략).

    노트는 사람이 스레드에 직접 남긴 판단이라 이 앱에서 가장 비싼 자산이다
    (2026-08-11). 선정은 결정론(AI 콜 0): 직접 지목
    (allow — 메일 분석·쟁점 분석) 스레드의 노트는 무조건 싣고, 나머지는 질문
    토큰과 본문 겹침 순 상위 ASK_NOTES_TOP. 겹침 0이면 안 싣는다 — 무관한
    노트는 소음이다. 숨긴 스레드 것은 제외(allow 예외는 deny 가 이미 반영).

    '인용 금지'의 강제는 코드가 한다 — 노트는 정독 본문이 아니라 노트 문장을
    인용해도 검증(_quote_ok)에서 떨어진다."""
    rows = store.notes_all()
    if not rows:
        return ""
    allow = allow or set()
    picked, seen = [], set()
    for r in rows:
        if r["thread_id"] in allow and (r["content"] or "").strip():
            picked.append(r)
    ...
    scored = []
    for r in rows:
        if r["thread_id"] in seen or r["thread_id"] in deny:
            continue
        hay = f"{r['content']} {r['norm_subject'] or ''}".lower()
        n = sum(1 for t in toks if t in hay)
        if n and (r["content"] or "").strip():
            scored.append((n, r["mtime"], r))
    scored.sort(key=lambda x: (x[0], x[1]), reverse=True)
    picked += [r for _, _, r in scored[:max(0, ASK_NOTES_TOP - len(picked))]]
    if not picked:
        return ""
    lines = "\n".join(
        "- [#%d] %s\n%s" % (
            r["thread_id"], r["norm_subject"] or "",
            _note_for_prompt(r["content"])[:ASK_NOTE_CHARS])
        for r in picked)
    return ("\n[내 노트 — 사용자가 스레드에 직접 쓴 기록. 문맥 전용, 인용 금지"
            " — 모든 인용 근거는 정독 본문에서만]\n" + lines + "\n")


def _knowledge_block(store: Store, question: str, deny: frozenset) -> str:
    """승인된 지식(vault/knowledge 색인) 중 관련분 — 문맥 전용 블록.

    지식은 사람이 [지식으로 저장]을 눌러 확정한 조직 노하우라 노트와 같은 급의
    자산이다(2026-08-14). 선정은 같은 결정론(AI 콜 0): 질문 토큰과
    제목·본문 겹침 순 상위 ASK_KNOWLEDGE_TOP, 겹침 0이면 블록 생략.
    참조 스레드가 전부 숨김이면 뺀다(CLAUDE.md 3 — 재료 조립 전에 거른다).

    '인용 금지'의 강제는 코드가 한다 — 지식 문장은 정독 본문이 아니라 인용해도
    검증(_quote_ok)에서 떨어진다. 노트와 같은 계약."""
    ...
    scored = []
    for r in rows:
        tids = [int(t) for t in (r["threads"] or "").split(";") if t.strip()]
        if tids and all(t in deny for t in tids):
            continue
        hay = f"{r['title']} {r['content']}".lower()
        n = sum(1 for t in toks if t in hay)
        if n and (r["content"] or r["title"]):
            scored.append((n, r["mtime"], r))
    scored.sort(key=lambda x: (x[0], x[1]), reverse=True)
    picked = [r for _, _, r in scored[:ASK_KNOWLEDGE_TOP]]
    if not picked:
        return ""
    lines = "\n".join(
        f"- {r['title']}\n{(r['content'] or '')[:ASK_KNOWLEDGE_CHARS]}"
        for r in picked)
    return ("\n[지식 — 사용자가 승인해 저장한 조직 노하우. 문맥 전용, 인용 금지"
            " — 모든 인용 근거는 정독 본문에서만]\n" + lines + "\n")


def context_text(store: Store, cfg: Config, question: str) -> str:
    """`ask --context` — 엔진이 이 질문에 실을 결정론 문맥을 **같은 함수**로 보여 준다.

    왜 있는가: `/mail-research` 는 엔진을 거치지 않고 세션이 직접 분석하는데,
    그러면서 엔진이 매 호출마다 읽어 주던 셋(사용자 지침 · 내 노트 · 지식)을
    통째로 놓쳤다(2026-08-21). 노트·지식은 사람이 직접 쓴 판단이라 가장 비싼
    재료다. 에이전트가 파일을 직접 찾게 하면 홈 해석(MAILKB_HOME)·주석 제거·
    숨김 제외를 각자 흉내 내야 하므로, 엔진과 글자까지 같은 것을 명령으로 낸다.

    AI 호출 0. 재색인(notes/knowledge reindex)은 엔진과 똑같이 돌린다 — 방금
    고친 노트가 옛 내용으로 나오는 조용한 불일치를 막기 위해서다(색인은 파일의
    미러라 쓰기라 해도 되돌릴 수 없는 것이 없다). 비어 있으면 침묵하지 않고
    **왜 비었는지**(경로·색인 건수)를 말한다 — 그게 `<home>` 혼란의 실제 해법이다.
    """
    ...
    if rules:
        out.append("[사용자 지침 — 우선 적용]\n" + rules)
        full = len(cfg.ai_rules_text(limit=None))        # 같은 함수, 상한만 없이
        if full > len(rules):
            out.append(f"(지침 {full:,}자 — 엔진은 앞 {len(rules):,}자만 싣는다)")
    else:
        out.append(f"[사용자 지침]  없음 — {cfg.home / 'ai-rules.md'} 에 쓰면 실린다")

    # 안내에 쓰는 건수는 **보이는 것만** 센다 — 숨긴 스레드의 노트를 세어 놓고
    # "겹치는 말이 없다"고 하면 실제 이유(숨김 제외)를 가린다.
    n_notes = sum(1 for r in store.notes_all() if r["thread_id"] not in deny)
    n_know = 0
    for r in store.knowledge_all():
        tids = [int(t) for t in (r["threads"] or "").split(";") if t.strip()]
        if not (tids and all(t in deny for t in tids)):
            n_know += 1
    if q:
        notes = _notes_block(store, q, deny).strip()
        know = _knowledge_block(store, q, deny).strip()
        why_n = "노트 없음" if not n_notes else f"색인 {n_notes}건 — 질문과 겹치는 말이 없다"
        why_k = "지식 없음" if not n_know else f"색인 {n_know}건 — 질문과 겹치는 말이 없다"
        out.append(notes or f"[내 노트]  관련 0건 ({why_n})")
        out.append(know or f"[지식]  관련 0건 ({why_k})")
    else:
        out.append(f"[내 노트]  색인 {n_notes}건 · [지식]  색인 {n_know}건 — "
                   "질문을 주면 관련분을 고른다: ask --context '<질문>'")
    return "\n\n".join(out)


def _note_for_prompt(content: str) -> str:
    """노트 본문 → 프롬프트용 여러 줄.

    _norm_ws(공백 **제거**)를 쓰면 안 된다 — 그건 인용 대조용이라 한글 문장이
    '온디바이스캐시전략은보류다'로 뭉쳐 모델이 읽기 어렵고 토큰만 낭비한다
    (2026-08-11 프롬프트 실측에서 발견). 줄 구조는 살리되 빈 줄과 템플릿의
    안 채운 불릿('- ')만 걷어낸다 — 안 쓴 항목을 실으면 '비어 있음'을 사실로
    오해할 여지가 생긴다."""
    out = []
...
def ask(store: Store, cfg: Config, question: str, backend: str | None = None,
    ...
    # 프롬프트 상단의 고정 문맥 — 지식(승인 암묵지)·내 노트 + 사용자 지침.
    # 조사·답변 콜에만 싣는다. 검증(VERIFY)·재작성(REPAIR)에는 넣지 않는다:
    # 검증기의 "외부 지식과 상식은 쓰지 않는다" 계약을 문맥 주입이 흔들면 안 된다.
    # 색인은 파일을 따라가므로 조회 전에 한 번 맞춘다.
    from . import knowledge as knowledge_mod
    from . import notes as notes_mod
    notes_mod.reindex(cfg, store)
    knowledge_mod.reindex(cfg, store)
    memory = _notes_block(store, q, deny, allow_tids)
    memory += _knowledge_block(store, q, deny)
    rules = cfg.ai_rules_text()
    rules_block = f"\n[사용자 지침 — 우선 적용]\n{rules}\n" if rules else ""
    ...
    out = {
        ...
        "open": open_items,
        # 재작성 전 원 답변(res)에서 꺼낸다 — answer 가 재작성·폴백으로 갈려도
        # 쟁점 골격은 살아남는다. claims 는 검증 통과분이라 근거 0 규칙이 걸린다.
        "issues": _clean_issues(res.get("issues"), claims),
...
# 인물 브리핑 — 답변 골격은 종전대로 두고 **시간 좌표만 더한다**(2026-08-18).
# `answer`·`headline` 의 모양을 가이드로 바꾸려 네 번 시도했으나 매번 사실 나열로
# 돌아왔다 — 상위 프롬프트가 두 필드를 이미 규정하고, 가이드는 그 아래 한 줄이라
# 힘이 모자란다. 형태를 바꾸려면 쟁점 분석의 issues[] 처럼 **일급 필드**가 있어야
# 한다(별도 과제). 반면 시간 좌표는 한 번에 먹었다: 6개월 창이라 3개월 전
# '회신 대기'가 현재형으로 읽히던 것이, '아직 유효한지 확인 필요'로 바뀌었다.
_BRIEF_GUIDE = """- 이 답변은 **인물 브리핑**이다. 다음 순서로 쓰라:
  ① 지금 걸려 있는 것(내 회신·결정을 기다리는 것) ② 진행 중인 일 ③ 이 사람의 일하는 방식.
  ①이 없으면 없다고 쓰라 — 지어내지 마라.
- {recency}
  **오래된 사실을 현재처럼 쓰지 마라.** 그 사이 회의·결정으로 끝났을 수 있는 것은
  단정하지 말고 '열린 것'에 "…이 아직 유효한지 확인 필요"로 보내라."""
...
def _recency_line(store: Store, ids: list[int], today: str | None = None) -> str:
    """브리핑 재료의 시간 좌표 한 줄 — 마지막 교신일과 경과일.

    창이 6개월이라 **가장 최근 메일도 오래됐을 수 있다.** 그 사실을 모델이 모르면
    3개월 전 '회신 대기'를 현재형으로 쓴다(스레드 진단 1차에서 기각 12/21 이
    전부 그 종류였다). 결정론 값이라 모델 협조에 기대지 않는다.
    """
    rows = store.messages_by_ids(ids[:1])
    last = (rows[0]["sent_on"] or "")[:10] if rows else ""
    if not last:
        return "재료의 시점을 알 수 없다 — 오래된 사실을 현재처럼 쓰지 마라."
    try:
        ...
        return "재료의 시점을 알 수 없다 — 오래된 사실을 현재처럼 쓰지 마라."
    if gap <= 2:
        return f"마지막 교신은 {last}(오늘 기준 {gap}일 전)로 최신이다."
    return (f"**마지막 교신이 {last} 로 {gap}일 전이다.** 이 브리핑의 재료는 "
            f"그 시점까지이고, 그 뒤 상황은 메일에 없다.")


def brief_question(who: str, months: int = BRIEF_MONTHS) -> str:
    """인물 브리핑의 질문 — 캐시 키의 재료라 **여기 한 곳**에서만 만든다
    (mail_question·thread_question 과 같은 이유). `ask --context --person` 도
    이걸 써야 엔진이 실을 문맥과 같은 질문으로 고른다."""
    return f"{(who or '').strip()} · 최근 {months}개월 브리핑 — 내가 알아야 할 것"
...
def brief(store: Store, cfg: Config, addr: str, name: str = "",
    ...
    q = brief_question(who, months)
    res = ask(store, cfg, q, backend=backend, use_cache=use_cache,
              progress=progress, today=today, seed_ids=ids,
              scope_key=(addr or "").strip().lower(),
              guide=_BRIEF_GUIDE.format(recency=_recency_line(store, ids, today)),
              on_event=on_event, cancel=cancel)
...
def thread_question(tid: int, subject: str) -> str:
    """쟁점 분석의 자동 질문 — 캐시 키의 재료라 **여기 한 곳**에서만 만든다
    (mail_question 과 같은 이유 — 웹 제출과 엔진이 각자 만들면 캐시가 갈라진다)."""
    subj = (subject or "(제목 없음)").strip()[:60]
    return f"스레드 #{int(tid)} ({subj}) — 쟁점 분석: 쟁점별 입장과 현재 상태"


_THREAD_GUIDE = """- 이 답변은 **스레드 쟁점 분석**이다. 분석의 골격은 answer 가 아니라 **issues** 에 담아라
  — answer 는 검증 후 재작성될 수 있지만 issues 는 그대로 남는다:
  "issues": [{"title": "쟁점명(짧게)", "status": "합의|해소|진행 중|보류|평행선",
              "note": "누가 제기했고 무엇으로 정리됐는지 1~2문장"}]
- 쟁점 등록 기준 — 셋 다 만족할 때만: ① 결론을 요구하며 제기됐다(결정 요청·선택지·
  문제 보고) ② 누군가 다뤘다(입장·반론·확정·진행) ③ "합의됐나/해결됐나/아직인가"를
  물을 수 있다. 이견 없이 확정된 결정도 쟁점이다. 단순 공유·통보·인사·수신 확인·
  다른 쟁점의 진행 보고·스레드의 발단 보고는 쟁점이 아니다(경위·배경으로 보내라).
- status: 의견이 갈렸던 쟁점이면 합의/평행선/보류, 사건·작업이면 해소/진행 중.
  확실치 않으면 빈 문자열로 두라. 쟁점이 하나뿐이면 하나만 — 억지로 나누지 마라.
- 각 claim 에 "issue": 쟁점 번호(1부터)를 달아 근거를 쟁점에 연결하라.
- answer 는 스레드 경위 요약 2~4문장. 이 스레드 밖의 사실은 쓰지 마라."""


def map_thread(store: Store, cfg: Config, tid: int,
               backend: str | None = None, use_cache: bool = True,
               progress=None, today: str | None = None,
               on_event=None, cancel=None) -> dict:
    """스레드 쟁점 분석 — 같은 조사 엔진에 스레드 전체를 심고 범위를 잠근다.

    인물 브리핑처럼 lock_scope=True: 쟁점 분석의 계약은 '이 스레드 안에서 누가
    무엇을 말했나'다. 범위가 열리면 다른 스레드의 문장이 '사람별 입장' 인용으로
    통과해 분석이 오염된다(인용 검증은 정독 본문 대조라 스레드 밖 정독을 막지
    못한다). 결과는 ask_cache 영구 저장(scope=thread:<id>) — 분석 이력에
    나타나고 이어 묻기·삭제가 그대로 된다."""
    tid = int(tid)
    msgs = store.thread_messages(tid)
    if not msgs:
        raise review.AIError(f"스레드 #{tid} 을 찾을 수 없습니다")
    # SEED_MAX(60통) 절단은 앞부분만 남긴다 — 최초 5통(쟁점의 기원·최초 입장)과
    # 최신 나머지(현재 상태)를 남기고 중간을 비운다. 중간은 잠긴 범위 안에서
    # 라운드 루프의 검색이 회수한다.
    ids = [int(m["id"]) for m in msgs]
    if len(ids) > SEED_MAX:
        head = ids[:min(5, SEED_MAX)]
        rest = SEED_MAX - len(head)
        ids = head + (ids[len(head):][-rest:] if rest > 0 else [])
    subj = (msgs[0]["subject"] or "(제목 없음)").strip()[:60]
    q = thread_question(tid, msgs[0]["subject"] or "")
    # 대상 스레드는 숨김이어도 조사한다(allow_tids) — 그 스레드의 [쟁점 분석]을
    # 직접 눌렀다는 것이 곧 명시 의도다(analyze_mail 과 같은 판례). 확장이 없는
    # 잠긴 범위라 다른 숨김 스레드는 애초에 들어오지 않는다.
    res = ask(store, cfg, q, backend=backend, use_cache=use_cache,
              progress=progress, today=today, seed_ids=ids,
              scope_key=f"thread:{tid}", guide=_THREAD_GUIDE,
              lock_scope=True, seed_noise=True, allow_tids={tid},
              on_event=on_event, cancel=cancel)
    res["thread"] = {"tid": tid, "subject": subj, "n": len(msgs)}
    return res
...
def _stale_of(store: Store, key: str) -> int:
    ...
    **세어야 한다. 빼면 안 된다**(2026-08-11). 종전에는 두 rowid 의 차를 그대로
    개수로 썼는데, 번호가 날짜 기반이 되면서 하루만 넘어가도 차이가 100,000 이
    된다(모의 검증에서 3 이어야 할 값이 1,200,000 이었다). id 는 신원이지
    카운터가 아니다.
    """
    basis = basis_of(key)
    return store.count_after(basis) if basis is not None else 0
...
def conversations(store: Store, limit: int = 40) -> list[dict]:
    ...
    for rid, members in groups.items():
        ...
        convs.append({
            ...
            # 빼지 않고 센다 — _stale_of 의 주석 참고
            "stale": store.count_after(seen) if seen is not None else 0,
