...
decisions(장기기억) · ask_cache(분석 이력) · people_dossier(인물 요약) ·
...
class Store:
    ...
    def _ensure_people_dossier_schema(self) -> None:
        ...
        """
        bcols = {r["name"] for r in
                 self.db.execute("PRAGMA table_info(reclean_backup)")}
        if bcols and "from_version" not in bcols:
            try:                       # 구 DB — 컬럼만 얹는다(재수집 불필요)
                self.db.execute("ALTER TABLE reclean_backup "
                                "ADD COLUMN from_version INTEGER DEFAULT 0")
    ...
    def _reclean_quotes(self) -> None:
        ...
        try:
            ...
            # 먼저 적재된 것)이다. id 는 적재 순서라 MIN(id) 가 정확히 그것 —
            # 발신 시각순으로 잡으면 **나중에 백필된 더 오래된 메일**이 first 로
            # 뽑혀, 진짜 첫 보유분의 유일한 인용 체인이 잘려 나간다
            # (--since 로 최근분 먼저 모은 뒤 --full 을 돌리는 실제 순서.
            #  2026-07-31 리뷰 실증).
            firsts: dict[int, int] = {}
            for r in self.db.execute(
                    "SELECT thread_id, MIN(id) AS mid FROM messages "
                    "GROUP BY thread_id"):
                firsts[r["thread_id"]] = r["mid"]
            changed = 0
            threads: set[int] = set()
    ...
    def ask_basis(self) -> int:
        ...
        row = self.db.execute("SELECT MAX(rowid) m FROM messages").fetchone()
    ...
    def person_thread_context(self, addr: str, limit: int = 8,
        ...
        rows = self.db.execute(
            """SELECT DISTINCT t.id, t.rolling_summary, t.last_date
               ...
               WHERE m.is_sent=0 AND m.sender_addr=? AND t.hidden=0
    ...
    # ------------------------------------------ 본문 HTML 수명주기 (이미지 프룬)
    # docs/ARCHITECTURE.md §6.1: retain_days 경과 시 표시용 HTML 을 텍스트 수준으로
