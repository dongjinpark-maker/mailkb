...
def sig_memory(store, cfg, d: dict) -> dict:
    ...
    왜 통계에 두나: `review._summary_window` 는 소급 상한이 `ai.summary_max_days`
    (기본 1일)라 **앱을 안 연 날의 메일은 요약·수확에서 영구히 빠진다.** 그 문단
    docstring 이 "3일 넘게 비운 구간의 가장 오래된 날은 요약에서 빠질 수 있음
    (의도된 트레이드오프)"이라고 스스로 적어 두었는데, **그 대가가 어디에도
    보이지 않았다.** 사용자는 자기 기억에 구멍이 몇 개인지 모른 채 상한을 올릴지
    말지 판단할 수 없다. 이 절은 허영 지표가 아니라 이미 있는 트레이드오프의
    계량기다(§4 받은 메일 구성이 노이즈 규칙을 점검하는 것과 같은 부류).

    날짜별 판정은 회고 파일의 AI 절 유무로 한다 — 파일 존재만 보면 거짓말이
    된다(자동 생성분은 ai=False). DB 쪽 두 숫자는 그 결과의 누적이다.
    """
    ...
    for wi, w in enumerate(d["weeks"]):
        for wd in range(7):
            ...
            days.append({"date": day, "on": on, "weekend": wd >= 5,
                         "wd": wd, "wi": wi})
    summed = {r["id"] for r in store.db.execute(
        "SELECT id FROM threads WHERE rolling_summary != ''")}
    tids = set(d["threads"])
    since = d["weeks"][0].isoformat()
    decided = store.db.execute(
        "SELECT COUNT(*) FROM decisions WHERE status='confirmed' AND "
        "COALESCE(NULLIF(decided_on,''), substr(created,1,10)) >= ?",
        (since,)).fetchone()[0]
    on_n = sum(1 for x in days if x["on"])
    sum_n = len(tids & summed)
    return {"days": days, "days_on": on_n, "days_total": len(days),
            "threads_sum": sum_n, "threads_total": len(tids),
            "decisions": decided, "any": bool(on_n or sum_n or decided)}
...
def _body(d: dict, cfg, store) -> str:
    ...
    if mem["any"]:
        ...
        <span class="memitem"><b>요약된 스레드 {mem["threads_sum"]}</b>
          <span class="dim">/ {mem["threads_total"]}</span></span>
        <span class="memitem"><b>확정된 결정 {mem["decisions"]}건</b></span>
      </div>
      {svg_coverage(mem["days"], d["n_weeks"])}
      <p class="memhint">빈 칸의 메일은 요약·수확에서 <b>영구히</b> 빠집니다 —
    ...
    sec5 = f"""<section class="card"><h2><span class="no">5</span>기억 커버리지
        {_win(f"최근 {tw}주")}</h2>
      <p class="desc">그날 회고에서 AI 계층(요약·수확)이 실제로 돌았는지 — 채운 칸이 지식이 쌓인 날입니다. 파일이 있는 것만으로는 세지 않습니다(자동 생성분은 AI 를 부르지 않습니다).</p>
      {sec5_body}</section>"""
