...
def deadline_signals(store: Store, cfg: Config,
    ...
    개인 액션 신호가 아니므로 제외. 반환 (thread_id, 제목, 문장) — thread_id 는
    데일리 '참고'에서 '지금 할 일'에 이미 붙은 기한(⏰)과의 중복 제거용.
    """
    signals = []
...
def deterministic(store: Store, cfg: Config, date_iso: str | None = None) -> dict:
    ...
    # 미답변 목록도 개입 큐와 같은 기준(액션이 걸린 것)으로 좁힌다 — 데일리
    # 본문에선 '지금 할 일'이 이미 그 스레드들을 다 보여주므로 별도 섹션은 없고
    # (2026-07-17 UI 재구성), 키는 다른 소비처 호환용으로 유지한다.
...
EXEC_SUMMARY = """당신은 담당자의 하루 메일을 요약한다. 아래는 오늘 메일에서 이미 추출된 사실들이다. 이것만 근거로 요약을 써라.

규칙 (한국어):
- 3~5문장, 최대 두 문단. 첫 문단 = 오늘 판도(무엇이 결정·확정·진행·이슈였나),
  둘째 문단 = 내 활동(회신·종결시킨 요청)과 남은 일 — 할 일이 있으면 최우선 1건을 기한과 함께 지목.
- 입력에 없는 사실·수치·스레드 번호를 만들지 마라. 스레드 언급엔 (#번호) 표기.
- 중요도 순으로 골라라 — 모든 항목을 언급할 필요 없다. 제목·머리말·인사말 금지, 요약 본문만.
...
def _exec_facts(det: dict) -> dict:
    ...
    todo = []
    for it in det.get("intervention", []):
        label = dict(CATEGORIES).get(it["category"], it["category"])
        dl = " · ⏰기한" if it.get("tag") else ""
        why = f" — {it['ai_reason']}" if it.get("ai_reason") else (
            f" — 「{it['snippet']}」" if it.get("snippet") else "")
        todo.append(f"[#{it['thread_id']}] {label} · {it['who']}: "
                    f"{it['subject']} ({day_label(it)}{dl}){why}")
    ...
    todo_ids = {it["thread_id"] for it in det.get("intervention", [])}
    flow = []
    for it in (det.get("digest") or {}).get("work", []):
        if it["thread_id"] in todo_ids:
            continue
    ...
    return {"changes": block(changes), "todo": block(todo),
            "activity": block(activity), "flow": block(flow)}


def ai_exec_summary(store: Store, cfg: Config, det: dict,
                    backend: str | None = None) -> str | None:
    """데일리 맨 위 '하루 요약'(3~5문장) 생성 — sonnet 1콜, graceful.

    입력은 det 의 추출 결과 한 줄들뿐이라 토큰이 바운드되고, 원문이 없어
    환각 표면이 좁다. 백엔드 미설정·호출 실패 시 None (render 가 결정론
    한 줄 통계로 대체)."""
    try:
        cmd = cfg.ai_cmd(backend)
    except SystemExit:
        return None
    try:
        out = ai_run(cmd, EXEC_SUMMARY.format(**_exec_facts(det)))
    except AIError:
        return None
...
def run_ai_layer(
    ...
    """AI 계층(요약 갱신→수확→디제스트→개입 분류→정리→하루 요약)을 graceful 하게 실행.
...
# ------------------------------------------------------------------ 렌더링
# 데일리 구성(2026-07-17 재구성): 독자의 질문 순서 — 하루 요약(맨 위) →
# 지금 할 일 → 오늘 확정·변경 → 오늘 흐름(중복 제외) → 참고(웹에선 접힘).
# 같은 스레드는 한 번만 상세히 — 파이프라인 단계별 출력을 그대로 쌓지 않는다.

_TODO_MARKS = {"decide": "🔴결정", "respond": "🟠회신",
               "stalled_mine": "🟡정체", "stalled_thread": "⚪정체"}


def render_intervention(lines: list[str], queue: list[dict]) -> set[int]:
    """'지금 할 일' — 그룹 헤더 없이 항목별 종류 마커(🔴결정 등)로 평탄화.
    정렬은 큐가 이미 우선순위순(AI 정리 반영). 데일리·CLI act 공용.
    반환: 노출한 thread_id (아래 섹션 중복 제거용)."""
    lines.append(f"## 지금 할 일 ({len(queue)}건)")
    if not queue:
        lines += ["- 없음", ""]
        return set()
    for it in queue:
        mark = _TODO_MARKS.get(it["category"], "")
        pri = f" [{it['ai_priority']}]" if it.get("ai_priority") else ""
        star = " ★" if it.get("personal") else ""
        dl = " · ⏰기한" if it.get("tag") else ""
        flag = f" · {it['ai_flag']}" if it.get("ai_flag") else ""
        lines.append(f"- {mark}{pri}{star} [#{it['thread_id']}] {it['who']}: "
                     f"{it['subject']} — {day_label(it)}{dl}{flag}")
        # 부연은 한 줄만 — AI 사유·제안이 있으면 그것, 없으면 근거 인용
        if it.get("ai_reason"):
            act = f" → {it['ai_action']}" if it.get("ai_action") else ""
            lines.append(f"  ↳ {it['ai_reason']}{act}")
        elif it.get("snippet") and it["category"] in ("decide", "respond"):
            lines.append(f"  「{it['snippet']}」")
    lines.append("")
    return {it["thread_id"] for it in queue}


def _render_changes(lines: list[str], det: dict) -> set[int]:
    """오늘 확정·변경 — 수확 델타 + 장기기억 제안 병합(비-AI 날은 통째 생략).
    델타 줄이 이미 언급한 스레드의 제안은 별도 줄 없이 말미 안내로만."""
    h = det.get("harvest")
    if not h:
        return set()
...
def _render_flow(lines: list[str], det: dict, shown: set[int]) -> None:
    """오늘 흐름 — 업무 스레드 중 위(할 일·확정)에 안 나온 것만."""
...
def _render_reference(lines: list[str], det: dict, shown: set[int]) -> None:
    ...
    # 기한 신호 중 '지금 할 일'에 이미 ⏰ 로 붙은 스레드는 제외 — 잔여만
...
def _stat_line(det: dict) -> str:
    """비-AI 데일리의 머리 한 줄 — 하루 부피와 최우선 1건."""
    q = det.get("intervention", [])
    line = (f"수신 {det.get('received_count', 0)} · 발신 {len(det.get('sent', []))}"
            f" · 할 일 {len(q)}")
    if q:
        top = q[0]
        dl = " · ⏰기한" if top.get("tag") else ""
        line += f" — 최우선: [#{top['thread_id']}] {top['subject']}{dl}"
    return line


def render(det: dict, ai_text: str | None = None) -> str:
    lines = [f"# {det['date']} 데일리 리뷰", ""]
    # 머리 요약 — AI 하루 요약(3~5문장), 없으면 결정론 한 줄. 웹은 첫 ## 이전
    # 문단을 요약 카드로 스타일링한다.
    head = (det.get("exec_summary") or "").strip()
    lines += [head or _stat_line(det), ""]

    shown = render_intervention(lines, det.get("intervention", []))
