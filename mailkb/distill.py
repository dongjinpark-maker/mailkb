...
설계(docs/ARCHITECTURE.md §6.6): 데일리 AI 의 임무는 통찰 생산이 아니라 **수확** —
...
def harvest(store: Store, cfg: Config, det: dict,
            backend: str | None = None, on_event=None, cancel=None) -> dict | None:
    """데일리 수확 — 지난 수확 이후 업무 스레드에서 결정 후보·신호를 추출해 원장
...
def _dossier_materials(store: Store, cfg: Config, addr: str,
    ...
    threads = "\n".join(
        ...
        + "\n".join(
            ...
            for e in c["excerpts"])
        for c in ctx) or "(없음)"
    # 결정·신호도 숨긴 스레드 것은 뺀다 — 제목 한 줄이라도 숨긴 대화의 내용이다
    deny = store.hidden_thread_ids()
    decs = [d for d in store.person_decisions(addr, name)
            if d["thread_id"] not in deny]
